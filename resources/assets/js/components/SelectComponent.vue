  ...
  <div class="select">
	<div class="select__combobox">
	  ...
	  <div class="selected__options" @click="openClose">
		<slot v-if="selected.length === 1" name="selectedLabel" :option="getOptionData(selected[0])">
		  <span class="selected__text">{{ getOptionData(selected[0]).label }}</span>
		</slot>
		<div v-else-if="selected.length > 1" class="selected__text">
		  <slot
			   v-for="tag in selected"
			   name="tag"
			   :tag="getOptionData(tag)"
			   :selected="selected"
		  >
            <span class="selected__tag">
              {{ getOptionData(tag).label }}
              <i @click="removeFromSelected(tag)" class="fal fa-times"></i>
            </span>
		  </slot>
		</div>
		<span v-if="selected.length === 0" class="selected__text placeholder">{{ placeholder }}</span>
		<div class="select__actions">
		  <inline-svg class="select__arrow" src="public/images/icons/arrow-right.svg"/>
		</div>
	  </div>
		  ...
		  <ul class="options">
			<li
				 ...
				 :class="{
				 	...
				 	'disabled': maxSelections === selected.length && !selected.includes(option.value)
				 }"
